...
GKZ A-configurations, probing three levels of equivalence:
  ...
  1. Unimodular equivalence (GL_n(ℤ) + translation) — same GKZ D-module.
  2. Affine equivalence (GL_n(ℚ) + translation) — same combinatorial type.
...
Interpretation:
  ...
  det(M) = 2, so M is NOT in GL₃(ℤ) and the map is NOT unimodular.
""")
...
A finite-index map M with |det(M)| = 2 means:
  • The image M(ℤ³) is a sublattice of index 2 in ℤ³.
  ...
  • Their GKZ D-modules are related but not isomorphic over ℤ — they have the
...
print("""
Observation:
  The intrinsic coordinates are expressed in the greedy ℤ-basis chosen from
  ...
  The Smith invariants are basis-independent invariants:
    Triangle [1,1,1] → spans ℤ³ (full lattice, index 1).
    Triple-K [1,1,2] → last generator scaled by 2 (sublattice, index 2 in ℤ³).
...
This is not a coincidence: both polytopes are cross-polytopes (octahedra) in ℝ³.

  Triangle centered at (1/2, 1/2, 1/2):  vertices are three antipodal pairs
    ±(1/2,1/2,-1/2), ±(1/2,-1/2,1/2), ±(-1/2,1/2,1/2) in integer coordinates.

  Triple-K centered at (1, 1, 1):  vertices are exactly ±e₁, ±e₂, ±e₃ relative
...
group B₃ of order 48 (the symmetry group of the octahedron / cube), so both
...
abstract convex body.  What distinguishes them is purely their lattice embedding:
  • Triangle: centred at a half-integer point; its vertices span ℤ³ (Smith [1,1,1]).
...
of the GKZ D-module for generic parameters β (Klausen 2020, Theorem 2.2).
...
for a Feynman integral it counts the master integrals at generic kinematics.
  ...
  Triangle   : vol = {vol_tri}  (4 independent Γ-series / master integrals)
  Triple-K   : vol = {vol_tpk}  (same — the index-2 sublattice rescales the measure)
  Banana-3   : vol = {vol_ban}
  4-simplex  : vol = {vol_s4}

Triangle and triple-K have the same normalized volume even though their
...
spans a sublattice of index 2, so the lattice-normalised volume is
  vol_0 = 3! × Euclidean_volume / sublattice_index = 6 × (4/3) / 2 = 4.
The two GKZ systems are isomorphic over ℚ and have the same holonomic rank.
""")
...
print(f"""  Configuration pair: Triangle  vs  Triple-K
  ...
  Affinely equivalent (over ℚ)?   {"YES" if _aff_same else "NO":<6}   {"Witness found" if _aff_same else "Not equivalent"}
  ...
  |Aut(P)| same?                  {_aut_same_s:<6}   {auts_tri.order} vs {auts_tpk.order} — both cross-polytopes (B₃)

  Physical meaning:
  • Both Newton polytopes are cross-polytopes (octahedra) with |Aut(P)| = 48 = B₃.
  ...
  • Smith invariants [1,1,1] ≠ [1,1,2] prove non-unimodular equivalence:
    the triangle spans ℤ³ while the triple-K spans the even-sum sublattice.
  ...
  • Normalized volume = {vol_tri} for both: same holonomic rank, same master-integral
    count at generic kinematics.  The GKZ systems are isomorphic over ℚ.
  • Over ℤ they are NOT isomorphic: the 2-torsion in the lattice quotient
    ℤ³ / (even-sum sublattice) ≅ ℤ/2 is the residual difference.
""")
