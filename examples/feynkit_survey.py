...
This script systematically processes four families of Feynman diagrams:
  ...
  Family 3 — Massless complete graphs K_n (n = 3, 4)
      ...
      K_5 is excluded: its normalized volume computation takes ~10 minutes.
...
For each diagram the script records:
  ...
  • GKZ β-parameters (Euler grading)
...
For every equivalence found the script exhibits:
  ...
  — the change of integration variables u_i = Σ_j M_ij v_j + t_i
  — the resulting GKZ β-parameter identity I_A(β, z) = I_A(T·β, z_P)
...
def _record_from_fi(
    ...
    _emit(
        ...
        f"A={A.rows}×{A.cols}, toric_gens={len(ti.generators)}, "
...
def _record_from_cfg(
    ...
    _emit(
        f"    done in {time.time() - t0:.1f}s  →  "
        f"A={A.rows}×{A.cols}, toric_gens={len(toric_gens)}, vol={cfg.normalized_volume}"
    )
    return rec
...
def main() -> None:
    ...
    _rule("-", 78)
    for r in records:
        shape = f"{r.a_matrix.rows}×{r.a_matrix.cols}"
        _emit(
    ...
    for er in all_equiv:
        ...
        _emit(f"  Linear map M  ({er.M.rows} × {er.M.cols}):")
        _print_matrix(er.M)
        _emit()
        ...
        # β-parameter identity (only for Feynman integral source records)
    ...
    _emit()
    _emit("  Pattern: n-gon toric count grows rapidly (~C(n,2) × something).")
