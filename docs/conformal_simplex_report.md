...
edge), giving an A-configuration in ℝⁿ.
...
- **n lower monomials**: ∏_{j≠i} u_j  (degree n−1)
- **n upper monomials**: u_i² ∏_{j≠i} u_j  (degree n+1)
...
edges.  Its LP A-configuration lives in ℝ^{C(n,2)}.  For n=3, K_3 = C_3 (the graphs
...
| n | config | N  | dim | Smith invariants | vol₀ | \|Aut\| |
|---|--------|----|-----|-----------------|------|---------|
...
spans the full integer lattice ℤ³, while the triple-K spans the even-sum sublattice
(an index-2 subgroup).

### n=4

C_4 LP (box) and BMS_4 live in the same ambient space ℝ⁴, but their monomial counts
...
K_4 LP lives in ℝ⁶ (C(4,2)=6 LP variables) vs BMS_4 in ℝ⁴ — ambient dimensions
...
- U = u₁u₂ + u₁u₃ + u₂u₃  (3 spanning-tree monomials, degree 2)
- F = s₁₂u₃ + s₁₃u₂ + s₂₃u₁  (3 kinematic monomials, degree 2)
- Total: N = 6
...
### Coincidence 3: Same normalised volume (vol₀ = 4)

Both C_3 and BMS_3 have normalised volume 4, meaning their GKZ systems have the same
holonomic rank: 4 independent A-hypergeometric series at generic β.

For n=4:
- vol₀(C_4) = 11 ≠ vol₀(BMS_4) = 8
...
- **Smith invariants**: [1, 1, …, 1, 2] — the last invariant is always 2, meaning the
  monomial support spans a codimension-1 even-sum sublattice (index 2 in ℤⁿ).
- **Normalised volume**: vol₀(BMS_n) = n! × 2ⁿ / (product of Smith invariants) → the
  holonomic rank grows rapidly with n.
- **Symmetry group**: |Aut(BMS_n)| = n! × 2ⁿ — the full hyperoctahedral group Bₙ acts
