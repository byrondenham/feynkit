...
the affine hull are lifted to primitive ambient forms mu . x <= beta, which
...
class Facet:
    ...
    lattice_index
        ...
        lattice_form is then primitive on ZA only when ZA = Z^(n+1).
    """
    ...
    def lattice_form(self) -> tuple[Fraction, ...]:
        ...
        is primitive on ZA only when ZA = Z^(n+1). For a lower-dimensional P it
        is defined modulo the affine-hull forms.
        """
...
def _lift_form(lift: _Lift, normal: Sequence[int], offset: int) -> tuple[tuple[int, ...], int, int]:
    """Lift the chart facet normal . c <= offset to (mu, beta, t), with mu . x <= beta.
    ...
    it is reduced modulo the forms vanishing on L, and beta = mu . o +
