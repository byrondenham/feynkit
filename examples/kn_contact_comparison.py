...
Note on K_5: computing the convex hull of 235 points in ℝ^{10} (the K_5 LP
...
print("""
Remove the I_n columns from the contact star (the "x̄_i = 1" restriction).
The remaining n columns carry exponent vectors {-e_i : i = 1..n} ⊂ ℝ^n,
which are the vertices of an (n-1)-simplex.

Question: is this simplex affinely equivalent to the triangle LP Newton polytope?

Triangle LP Newton polytope: ℝ^3, 6 vertices, norm-vol = 4.
...
Step 1 — n=3 sanity check:
  Triangle LP  : 4×6 A-matrix, ℝ^3, 6 verts, norm-vol = {vol_tri}
  Triple-K star: 4×6 A-matrix, ℝ^3, 6 verts, norm-vol = {vol_s3}
...
print(f"""
Step 4 — Cross-ratio reduction: conv{{-e_i in ℝ^n}} vs. triangle LP polytope:
  Triangle LP: ℝ^3, 6 vertices, norm-vol = {vol_tri}
  {'n':>2}  {'restricted polytope':>30}  {'First obstruction / result'}""")
print("  " + "─" * 62)
for n in [3, 4, 5]:
    r = step4[n]
    desc = f"ℝ^{n}, (n-1)-simplex, {n} verts"
...
Conclusion:
  ...
  For n ≥ 4: the -I_n simplex lives in ℝ^n ≠ ℝ^3, so the ambient-dimension
