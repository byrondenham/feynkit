...
NARRATIVE ARC
  Physics input (Feynman graph)
    ...
    -> GKZ hypergeometric system (A-matrix * Euler operators * toric operators)
    -> Newton polytope geometry (hull * volume * Smith invariants)
    -> Holonomic rank theorem (vol = rank for generic beta)
...
note(
    ...
    "generic beta and bounds the number of master integrals from above (Bitoun, "
    "Bogner, Klausen, Panzer 2019)."
)
...
print(f"""
  ...
  Toric generators   : {len(fi_tri.toric_ideal.generators)}  (toric operators, an analogue of IBP relations)

  Newton polytope    : {len(cfg_tri.newton_polytope_points)} hull vertices in R^{cfg_tri.ambient_dim}
  Normalised volume  : {cfg_tri.normalized_volume}  (= holonomic rank for generic beta)
...
note(
    ...
    "Gelfand-Kapranov-Zelevinsky 1990).  In the Feynman-integral context, when "
    "the exponent differences span Z^n, it bounds the number of master integrals "
    "from above, with equality for generic coefficients (Bitoun et al. 2019).  "
    "feynkit exposes vol(Delta_G) as "
...
note(
    ...
    "of the Newton polytope.  The bubble has volume 1, so rank 1 for generic "
    "beta.  Its toric ideal is trivial because the three columns of A are "
    "linearly independent; that by itself says nothing about the number of "
    "master integrals.",
    4,
)
...
# -----------------------------------------------------------------------------
# section 7  Toric ideal and toric operators
# -----------------------------------------------------------------------------
hdr(7, "Toric ideal and toric operators")

note(
    "The toric ideal I_A = ker(k[z_1,...,z_m] -> k[t^{+/-1}], z_j -> t^{a_j}) is "
    "generated by binomials z^{u_+} - z^{u_-} for each l = u_+ - u_- in ker_Z(A).  "
    "The corresponding operator d^{u_+} - d^{u_-} annihilates the generalised "
    "integral I_A(beta, z) with the z_j independent: an analogue of IBP relations "
    "(Chestnov et al. 2022), not an IBP relation itself.  The ideal has "
    "codimension dim ker_Q(A) = m - rank(A), so it needs at least that many generators."
)

ti = fi_tri.toric_ideal

sec("Triangle, toric generators")
...
sec("Consistency check: number of toric generators >= codimension m - rank(A)")
...
print(f"  # toric generators = {n_gen}   {'yes  (>= m - rank A)' if n_gen >= ker_dim else 'no'}")

sec("Bubble, trivial toric ideal")
...
sec("First symmetry pair, explicit identity")
s0 = sp_list[0]
print("  Linear map M =")
...
print(f"""
  {'Config':<28}  {'A':>8}  {'d':>3}  {'|V|':>4}  {'vol':>4}  {'Smith':<18}  {'|Aut|':>6}  {'#toric':>6}  Ref
  {'-'*100}""")

for label, fi_x, cfg_x, ref in all_entries:
    aut_x = cfg_x.automorphisms()
    toric_cnt = len(fi_x.toric_ideal.generators) if fi_x is not None else "-"
    print(
...
print("""
  Abbreviations:
    ...
    #toric = number of toric ideal generators, at least m - rank A (the codimension)
    -      = diagram not built as a FeynmanIntegral, so no toric count
